...
import {geoJson} from '@/api/users'
import { formData } from '@/utils/index'

export interface IMapState {
  provinces: object
  cityMap: object
  chinaGeoCoordMap: object
  currentMap: any
}
...
class Map extends VuexModule implements IMapState {
  ...
  @Mutation
  private SET_CURRENT_MAP(data: any){
    this.currentMap = data
  }
  
  @Action
  public SetCurrentMap(qydm: any){
    return new Promise((resolve)=>{
      geoJson(qydm).then(res=>{
        if((res as any).code === '200'){
          let json = JSON.parse(res.data.detail)
          this.SET_CURRENT_MAP(json)
          resolve(res)
        }
      })
    })
